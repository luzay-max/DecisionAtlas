...
DecisionAtlas 已经完成从 demo hardening 到 v0.3 平台化基线的主要推进。
...
latest commit: 899d0f6 docs: record april 28 validation update
```

关键缺口：

- `v0.3.0-rc.1` release 文档已经存在，但本地和远端 tag 仍只看到 `v0.2.1`。
- 真实 Postgres/Redis stack 和外部 hosted preview 仍需要在具备环境时重跑确认。
- 真实仓库决策质量仍是核心价值瓶颈，尤其是 source refs、thin candidates、why-search 命中质量。
