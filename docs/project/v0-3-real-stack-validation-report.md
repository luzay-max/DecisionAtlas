...
| Real stack explicit seeded reset | `powershell -ExecutionPolicy Bypass -File .\scripts\dev\start-real-stack.ps1 -ResetSeededDemo` | Restores the seeded guided demo lane before startup when prior walkthroughs consumed review/drift state. |
...
| Seeded demo readiness | `python scripts\demo\check_seeded_demo.py` | Checks accepted decisions, candidate queue, source refs, timeline-ready decisions, and open drift alert state for `demo-workspace`. |
...
| Seeded demo readiness hardening | `python -m pytest tests/test_seeded_demo_recovery.py tests/test_hosted_demo_operator_flow.py tests/db/test_migrations.py tests/governance/test_agent_guardrail.py -q` | Stage 8 targeted tests passed: readiness detects consumed candidate queue, reset restores walkthrough state, imported workspaces are preserved, real-stack reset option is documented, migration revision length guard passes, and guardrail summary output remains advisory. | pass | This is deterministic SQLite/operator-command validation; external hosted environment still requires operator rerun. | Rerun real-stack and hosted health/smoke with Docker/hosted URLs when available. |
