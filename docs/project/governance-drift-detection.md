...
The script can read accepted governance rules from the configured local SQLite database when available. For offline or test usage, pass `--rules-json`.
...
- `stale_rule`: deprecated, superseded, rejected, or draft governance guidance appears to be reused as active guidance.
