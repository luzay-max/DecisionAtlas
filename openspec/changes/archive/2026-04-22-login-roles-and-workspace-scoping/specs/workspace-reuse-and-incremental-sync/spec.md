...
#### Scenario: Authenticated actor without scope membership cannot see reusable workspace state
- **WHEN** an authenticated actor targets a repository but lacks membership in the current owner scope
- **THEN** repository lookup SHALL deny reusable workspace visibility instead of exposing workspace state

#### Scenario: Current owner scope is resolved before repository lookup
- **WHEN** an authenticated actor starts a repository lookup
- **THEN** the system SHALL resolve the current owner scope from session state before deciding whether an imported workspace is reusable
