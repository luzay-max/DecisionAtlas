...
#### Scenario: High retrieval score cannot override missing support
- **WHEN** a candidate answer has a high retrieval score but lacks accepted-decision fit, grounded refs, or same-thread chunk support for the asked rationale
- **THEN** the system SHALL keep the answer in a bounded unsupported state rather than returning `ok` or `limited_support`

### Requirement: Why support guidance reflects accepted candidate quality
The imported why experience SHALL account for accepted candidate quality, source-ref coverage, source URL/provenance gaps, and same-thread evidence when guiding users after the first imported baseline is accepted.
...
#### Scenario: Missing source URL or provenance keeps guidance bounded
- **WHEN** the accepted imported decision matches the question but has missing source URL or provenance support
- **THEN** the why experience SHALL preserve that limitation in answer context or guidance rather than presenting the result as broadly trustworthy
