...
The system SHALL expose drift evaluation state for imported workspaces so users can run it intentionally, interpret the result correctly, distinguish stronger supersession signals from broader review-only follow-up material, avoid repeated weak alerts that all represent the same accepted-decision thread, and avoid stale alert conclusions from earlier reevaluation passes.
