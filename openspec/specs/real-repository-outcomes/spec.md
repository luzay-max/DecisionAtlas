...
The system SHALL summarize imported-workspace readiness so users can tell whether a real repository run is ready for review, still evidence-limited, or ready for downstream why-search and drift actions.
...
- **WHEN** a live analysis run completes without enough accepted or candidate decisions to support meaningful downstream usage
...
The system SHALL treat imported why-answers as trustworthy only when they are grounded in accepted imported decisions with citations, and SHALL otherwise fail with an actionable explanation.
...
- **THEN** the system SHALL answer using those accepted decisions and SHALL return citations with the answer
